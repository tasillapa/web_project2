...
function chklogin() {
    if (!empty($_POST["username"]) && !empty($_POST["password"])) {
        ...
        $sql = "select * from ps_personnal as ps left join ps_profile as pp on ps.card_id = pp.card_id left join ps_class as pc on pc.class_id = pp.class_id"
                . " left join ps_position as ppo on pp.pos_id = ppo.pos_id left join ps_department as pd on pp.dep_id = pd.dep_id left join ps_level as pl on pp.lv_id = pl.lv_id left join ps_levelboss as lvb on pp.lvb_id = lvb.lvb_id where ps.username='" . $_POST["username"] . "' and ps.password='" . base64_encode($_POST["password"]) . "' and ps.status = '1'";
        $query = $cn->Connect->query($sql);
        if (mysqli_num_rows($query) >= 1) {
            while ($rs = mysqli_fetch_array($query)) {
                $_SESSION['card_id'] = $rs['card_id'];
                $_SESSION['username'] = $rs['username'];
                $_SESSION['password'] = base64_decode($rs['password']);
